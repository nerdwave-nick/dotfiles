...
if [ "$setkeyboard" == "0" ] ;then
    ...
    if gum confirm "Are you using a laptop and would you like to enable the laptop presets?"; then
        cp ./mlfw/templates/keyboard-laptop.conf ~/dotfiles/hypr/conf/keyboard.conf
        echo "source = ~/dotfiles/hypr/conf/layouts/laptop.conf" >  ~/dotfiles/hypr/conf/layout.conf
    elif [ $? -eq 130 ]; then
        echo ":: Installation canceled."
        exit 130
    else
        cp ./mlfw/templates/keyboard-default.conf ~/dotfiles/hypr/conf/keyboard.conf
    fi
    cp ./mlfw/templates/autostart.sh ~/dotfiles/qtile/autostart.sh
    ...
    #SEARCH="KEYBOARD_LAYOUT"
    #REPLACE="$keyboard_layout"
    #sed -i "s/$SEARCH/$REPLACE/g" ~/dotfiles/qtile/autostart.sh
